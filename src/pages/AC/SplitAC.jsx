...
import { Wind, Thermometer, Droplets, Zap, Power, Fan, Settings } from 'lucide-react';

const MOCK_SPLIT_ACS = [
  { id: 1, name: 'Main Office Split AC', location: '1st Floor', status: 'ON', mode: 'Cool', setTemp: 22, roomTemp: 23.5, fanSpeed: 'High', powerUsage: 1.2 },
  { id: 4, name: 'Manager Cabin', location: '1st Floor', status: 'OFF', mode: 'Fan', setTemp: 24, roomTemp: 26.0, fanSpeed: 'Low', powerUsage: 0.0 },
  { id: 5, name: 'HR Department', location: 'Ground Floor', status: 'ON', mode: 'Cool', setTemp: 21, roomTemp: 22.0, fanSpeed: 'Auto', powerUsage: 1.8 }
];

const SplitAC = () => {
  const [units, setUnits] = useState(MOCK_SPLIT_ACS);

  const togglePower = (id) => {
    setUnits(units.map(u => u.id === id ? { ...u, status: u.status === 'ON' ? 'OFF' : 'ON', powerUsage: u.status === 'ON' ? 0 : 1.5 } : u));
  };
    ...
    <div className="fade-in p-4 h-100" style={{ background: '#0b1121', minHeight: '100vh', fontFamily: "'Inter', sans-serif" }}>
      <div className="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
        <h4 className="text-white fw-black mb-1 d-flex align-items-center" style={{ letterSpacing: '1px' }}>
          <Wind className="me-2 text-info" size={28} />
          SPLIT AC CONTROL
        </h4>
      </div>
          ...
          <Col lg={4} md={6} key={unit.id}>
            <Card className="border-0 shadow-lg h-100 overflow-hidden" style={{ background: 'linear-gradient(145deg, rgba(30, 41, 59, 0.7) 0%, rgba(15, 23, 42, 0.9) 100%)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <Card.Body className="p-4">
                ...
                <div className="mt-3">
                  <Form.Label className="text-secondary fs-12 mb-2">Fan Speed</Form.Label>
                  <Form.Range />
                </div>
              </Card.Body>
            </Card>
