...
const LTRoom = ({ roomNumber: propRoomNumber }) => {
  const { id } = useParams();
  const roomNumber = propRoomNumber || id || '1';

  const [time, setTime] = useState(new Date());
  
  // Simulated data state
  const [metrics, setMetrics] = useState({
    voltage: { r: 235, y: 234, b: 236 },
    current: { r: 120, y: 118, b: 122 },
    powerFactor: 0.98,
    frequency: 50.0,
    totalLoad: 35.4,
    energy: 14502,
    breakers: [
      { id: 'INC-1', name: 'Main Incomer', status: 'ON', load: 85, type: 'ACB' },
      { id: 'OUT-1', name: 'Lighting DB', status: 'ON', load: 45, type: 'MCCB' },
      { id: 'OUT-2', name: 'HVAC Panel', status: 'ON', load: 78, type: 'MCCB' },
      { id: 'OUT-3', name: 'Server Room', status: 'ON', load: 60, type: 'MCCB' },
      { id: 'OUT-4', name: 'Pumps Panel', status: 'OFF', load: 0, type: 'MCCB' },
      { id: 'OUT-5', name: 'Spare', status: 'TRIP', load: 0, type: 'MCCB' }
    ]
  });
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setTime(new Date());
      // Jitter for realistic live data
      setMetrics(prev => ({
        ...prev,
        voltage: {
          r: 235 + (Math.random() * 4 - 2),
          y: 234 + (Math.random() * 4 - 2),
          b: 236 + (Math.random() * 4 - 2)
        },
        current: {
          r: 120 + (Math.random() * 10 - 5),
          y: 118 + (Math.random() * 10 - 5),
          b: 122 + (Math.random() * 10 - 5)
        },
        powerFactor: Math.min(1.0, Math.max(0.9, prev.powerFactor + (Math.random() * 0.02 - 0.01))),
        frequency: 50.0 + (Math.random() * 0.1 - 0.05),
        totalLoad: 35.4 + (Math.random() * 2 - 1)
      }));
    }, 2000);
    ...
    <div className="fade-in p-3 h-100 d-flex flex-column" style={{ background: '#0f172a', minHeight: '100vh' }}>
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 pb-3 border-bottom border-secondary border-opacity-25">
        <div>
          ...
          <p className="text-secondary fs-8 mb-0 uppercase tracking-widest">Low Tension Power Distribution & Monitoring</p>
        </div>
      </div>
          ...
          <Card className="border-0 h-100" style={{ background: 'rgba(30, 41, 59, 0.4)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <Card.Body className="p-4 d-flex flex-column justify-content-center">
              ...
              <div className="d-flex justify-content-between align-items-center mb-4 p-3 rounded-3" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                ...
                <span className="text-success fw-bold font-monospace fs-6">32°C</span>
              </div>
              <div className="d-flex justify-content-between align-items-center mb-4 p-3 rounded-3" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
              ...
              <div className="d-flex justify-content-between align-items-center p-3 rounded-3" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                <span className="text-white fw-bold text-uppercase fs-9">Last Maintenance</span>
                <span className="text-secondary fw-bold fs-9">12 Days Ago</span>
              </div>
            </Card.Body>
          </Card>
        </Col>
      ...
      <h5 className="text-white fw-bold text-uppercase fs-6 mb-3 d-flex align-items-center mt-2">
        ...
        Breaker Status (Outgoing Feeders)
      </h5>
      <Row className="g-4">
