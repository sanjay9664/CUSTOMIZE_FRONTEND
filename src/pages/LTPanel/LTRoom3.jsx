...
const LTRoom3 = () => {
  ...
  // Simulated data state for Room 3
  const [metrics, setMetrics] = useState({
    voltage: { r: 236, y: 235, b: 238 },
    current: { r: 80, y: 75, b: 82 },
    powerFactor: 0.99,
    frequency: 50.1,
    totalLoad: 25.4,
    energy: 11005,
    breakers: [
      { id: 'INC-3', name: 'Main Incomer 3', status: 'ON', load: 50, type: 'ACB' },
      { id: 'OUT-20', name: 'Critical IT Load', status: 'ON', load: 35, type: 'MCCB' },
      { id: 'OUT-21', name: 'UPS Input A', status: 'ON', load: 40, type: 'MCCB' },
      { id: 'OUT-22', name: 'UPS Input B', status: 'ON', load: 38, type: 'MCCB' },
      { id: 'OUT-23', name: 'Emergency Lights', status: 'ON', load: 15, type: 'MCB' },
      { id: 'OUT-24', name: 'Fire Pump Panel', status: 'OFF', load: 0, type: 'MCCB' },
      { id: 'OUT-25', name: 'Spare 1', status: 'OFF', load: 0, type: 'MCB' },
      { id: 'OUT-26', name: 'Spare 2', status: 'OFF', load: 0, type: 'MCB' }
    ]
  });
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setMetrics(prev => ({
        ...prev,
        voltage: {
          r: 236 + (Math.random() * 4 - 2),
          y: 235 + (Math.random() * 4 - 2),
          b: 238 + (Math.random() * 4 - 2)
        },
        current: {
          r: 80 + (Math.random() * 5 - 2),
          y: 75 + (Math.random() * 5 - 2),
          b: 82 + (Math.random() * 5 - 2)
        },
        powerFactor: Math.min(1.0, Math.max(0.9, prev.powerFactor + (Math.random() * 0.02 - 0.01))),
        frequency: 50.1 + (Math.random() * 0.1 - 0.05),
        totalLoad: 25.4 + (Math.random() * 1 - 0.5)
      }));
    }, 2000);
    ...
    <div className="fade-in p-3 h-100 d-flex flex-column" style={{ background: '#0f172a', minHeight: '100vh' }}>
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 pb-3 border-bottom border-secondary border-opacity-25">
        <div>
          ...
          <p className="text-secondary fs-8 mb-0 uppercase tracking-widest">Critical & IT Load Monitoring</p>
        </div>
      </div>
          ...
          <Card className="border-0 h-100" style={{ background: 'rgba(30, 41, 59, 0.4)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <Card.Body className="p-4 d-flex flex-column justify-content-center">
              ...
              <div className="d-flex justify-content-between align-items-center mb-4 p-3 rounded-3" style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                ...
                <span className="text-success fw-bold font-monospace fs-6">24°C</span>
              </div>
      ...
      <h5 className="text-white fw-bold text-uppercase fs-6 mb-3 d-flex align-items-center mt-2">
        ...
        Breaker Status (Outgoing Feeders - Room 3)
      </h5>
      <Row className="g-4">
